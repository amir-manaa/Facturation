...
import { IInvoiceItem } from '../../shared/interfaces';
...
export class InvoiceItemController {
  ...
  private initRoutes() {
    this.#router.get(
      `${this.#path}/invoice-item/:id`,
      isAuth,
      this.#getInvoiceItemById
    );
    this.#router.get(
      `${this.#path}/invoice-items/:invoiceId`,
      isAuth,
      this.#getInvoiceItems
    );
    this.#router.post(
      `${this.#path}/invoice-item`,
      isAuth,
    ...
    this.#router.delete(
      `${this.#path}/invoice-item/:invoiceItemId`,
      isAuth,
  ...
  async #getInvoiceItemById(
    ...
    try {
      ...
      const invoiceItem = await InvoiceItemService.getInvoiceItemById(
        invoiceItemId
      );
      return res
  ...
  async #updateInvoiceItem(
    ...
    try {
      ...
      const updatedInvoideItem = await InvoiceItemService.updateInvoiceItem(
        invoiceItemId,
        reqBody
      );
      return res
  ...
  async #deleteInvoiceItem(
    ...
    try {
      const invoiceItemId = req.params.invoiceItemId;
      const invoices = await InvoiceItemService.deleteInvoiceItem(
        invoiceItemId
      );
      return res
        .status(HTTP_RESPONSE_CODE.CREATED_201)
